...
bool PulseChiSqSNNLS::gpuDoFIt(DoFitArgs* parameters){
    ...
    return DoFit(args.samples, args.samplecor, args.pederr, args.bxs,
...
bool PulseChiSqSNNLS::Minimize(const SampleMatrix& samplecor,
  ...
  while (true) {
    ...
    if (iter >= maxiter) {
      //      edm::LogWarning("PulseChiSqSNNLS::Minimize") << "Max Iterations
      //      reached at iter " << iter <<  std::endl;
      //       std::cout << " maxiter =  " << iter << " :: " << maxiter <<
      //       std::endl;
      break;
    }
